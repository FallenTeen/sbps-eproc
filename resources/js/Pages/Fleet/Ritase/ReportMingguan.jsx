...
export default function ReportMingguan({ mulai, selesai, perHari, total_rit, total_upah }) {
    const [start, setStart] = useState(mulai);
    const [end, setEnd] = useState(selesai);

    const handleCari = () => {
        router.get(route("fleet.ritase.report-mingguan"), { mulai: start, selesai: end });
    };

    return (
        <AuthenticatedLayout>
            <Head title="Laporan Ritase Mingguan" />

            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Laporan Ritase Mingguan</h1>
                <p className="mt-1 text-sm text-gray-500">Rekap ritase per hari dalam periode</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <p className="text-sm text-gray-500">Total Rit</p>
                    <p className="text-xl font-bold text-gray-900">{Number(total_rit).toLocaleString("id-ID")}</p>
                </div>
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                    <p className="text-sm text-gray-500">Total Upah</p>
                    <p className="text-xl font-bold text-gray-900">Rp {Number(total_upah).toLocaleString("id-ID")}</p>
                </div>
            </div>
                        ...
                        {perHari.length === 0 ? (
                            ...
                            perHari.map((h, i) => (
                                <tr key={i}>
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-900 text-right">{h.total_rit}</td>
                                    <td className="px-6 py-4 text-sm text-gray-900 text-right">
                                        Rp {Number(h.total_upah).toLocaleString("id-ID")}
                                    </td>
                                </tr>
                            ))
                        )}
