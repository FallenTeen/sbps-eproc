...
export default function Create({ unitBisnis }) {
    const { data, setData, post, errors, processing } = useForm({
        unit_bisnis_id: unitBisnis.length === 1 ? unitBisnis[0].id : "",
        lokasi_asal: "",
        lokasi_tujuan: "",
        jarak_km: "",
        tarif_per_rit: "",
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("fleet.rute-tarif.store"));
    };

    return (
        <AuthenticatedLayout>
            <Head title="Tambah Rute Tarif" />

            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900">Tambah Rute Tarif</h1>
                <p className="mt-1 text-sm text-gray-500">Master tarif rute armada</p>
            </div>
