...
export default function Edit({ mixDesign, bahanBakus }) {
    const { data, setData, put, processing, errors } = useForm({
        mutu_beton:  mixDesign.mutu_beton || "",
        nama:        mixDesign.nama || "",
        deskripsi:   mixDesign.deskripsi || "",
        items:       (mixDesign.items || []).map(item => ({
            id:            item.id,
    ...
    const addItem = () => {
        setData("items", [...data.items, { id: null, bahan_baku_id: "", jumlah_per_m3: "" }]);
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route("production.mix-design.update", mixDesign.id));
    };

    return (
        <Layout>
            <div className="flex items-center gap-4 mb-6">
                <Link href={route("production.mix-design.show", mixDesign.id)} className="text-gray-400 hover:text-gray-600 transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                </Link>
                <h1 className="text-2xl font-bold text-gray-900">Edit Template: <span className="text-blue-600">{mixDesign.mutu_beton}</span></h1>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <FormField label="Mutu Beton" required error={errors.mutu_beton}>
                                <input type="text" value={data.mutu_beton} onChange={(e) => setData("mutu_beton", e.target.value.toUpperCase())}
                                    className={inputCls(errors.mutu_beton)} />
                            </FormField>
                            <FormField label="Nama Template" error={errors.nama}>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <Link href={route("production.mix-design.show", mixDesign.id)}
