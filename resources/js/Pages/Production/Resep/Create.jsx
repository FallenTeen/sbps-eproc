...
export default function Create({ produk, bahanBakus }) {
    const { data, setData, post, processing, errors } = useForm({
        bahan_baku_id:         "",
        jumlah_per_unit_output: "",
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("production.resep.store", produk.id));
    };
    ...
    return (
        <Layout>
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Tambah Item Resep</h1>
                    <p className="text-sm text-gray-500 mt-0.5">Produk: <span className="font-medium text-blue-600">{produk.nama}</span></p>
                </div>
            </div>

            <div className="max-w-xl">
                <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
                    <h2 className="text-base font-semibold text-gray-800 pb-3 border-b border-gray-100">Komposisi Bahan Baku</h2>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                            Jumlah per {produk.satuan_output} Output <span className="text-red-500">*</span>
                        </label>
                        <input type="number" step="0.0001" min="0.0001" value={data.jumlah_per_unit_output}
                            ...
                            placeholder="Contoh: 0.35 (m3 semen per m3 beton)"
                        ...
                        <p className="text-xs text-gray-500 mt-1">Satuan mengikuti bahan baku yang dipilih. Misal: 350 (kg/m³) atau 0.5 (m³/m³).</p>
                    </div>
